...
export function SignalDetail({ signal, trend }: { signal: Signal; trend: TrendPoint[] }) {
  return <main className="detail-page"><div className="detail-nav"><Link href="/"><ArrowLeft size={16} />Back to alert queue</Link><span className={`severity ${signal.severity}`}><i />{signal.severity.toUpperCase()}</span></div>
    ...
    <section className="evidence-metrics"><article><span>Observed</span><strong>{signal.observed.toLocaleString()}</strong><small>requests on signal date</small></article><article><span>Expected</span><strong>{signal.expected.toFixed(1)}</strong><small>matching-weekday median</small></article><article className="accent"><span>Observed / expected</span><strong>{signal.display_effect}</strong><small>{signal.uncertainty}</small></article></section>
