...
export function ExploreClient({ snapshot }: { snapshot: Snapshot }) {
  ...
  const setDistrict = (value: string) => { setDistrictState(value); const search = new URLSearchParams(window.location.search); if (value) search.set("district", value); else search.delete("district"); window.history.pushState({}, "", search.size ? `/explore?${search}` : "/explore"); };
  ...
  const selectedTrend = district ? snapshot.trends.by_district[district] ?? [] : snapshot.trends.citywide_volume;
  ...
  return <main className="explore-page" data-app-ready={hydrated ? "true" : "false"}><header className="page-intro"><p className="eyebrow">AGGREGATE EXPLORER</p><h1>Move from the city to one district.</h1><p>Inspect the spatial distribution of the evaluated problem categories. Exact request locations are intentionally unavailable.</p></header>
    ...
    <section className="explore-trend"><TrendChart points={selectedTrend} title={district ? `${district} evaluated category volume` : "Citywide evaluated category volume"} /></section>
