...
export function SignalCard({ signal, index }: { signal: Signal; index: number }) {
  return <article className="signal-card">
    <div className="signal-meta"><span className={`severity ${signal.severity}`}><i aria-hidden="true" />{signal.severity === "research_flag" ? "RESEARCH" : signal.severity.toUpperCase()}</span><span>{String(index + 1).padStart(2, "0")}</span></div>
